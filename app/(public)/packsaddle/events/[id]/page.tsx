...
export default async function PacksaddleEventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const supabase = await createServerSupabaseClient();
  const [event, { data: property }] = await Promise.all([
    getPublicEvent(supabase, id),
    getPublicPropertyBySlug(supabase, "packsaddle"),
  ]);
  ...
  const infoRows: Array<[string, ReactNode]> = event.startAt
    ? [
        ...
        ["Location", event.location || property?.name || "Packsaddle Precision"],
      ]
    : [
  ...
  return (
    <main>
      <div className="mx-auto max-w-property-max px-property-gutter pt-10">
        <p className="font-property-label text-[13px] uppercase tracking-[0.15em] text-property-ink">
          <Link href="/packsaddle/events" className="transition-colors hover:text-property-accent">
            Events Calendar
          </Link>
        ...
        <section className="grid grid-cols-1 items-stretch gap-12 lg:grid-cols-12">
          ...
          <div className="flex flex-col lg:col-span-5">
            {(event.type || event.discipline) && (
              <p className="mb-3 font-property-label text-[13px] uppercase tracking-[0.2em] text-property-accent-dark">
                {[event.type, event.discipline].filter(Boolean).join(" · ")}
              </p>
            )}
            <div className="mb-3 flex flex-wrap gap-2">
              {event.includedWithMembership && (
                <span className="inline-block border border-property-ink/20 px-3 py-1 font-property-label text-[11px] uppercase tracking-widest text-property-ink">
            ...
            <div className="space-y-3 border-y border-property-ink/10 py-6 mt-6">
              {infoRows.map(([label, value]) => (
                <div key={label} className="flex items-baseline gap-4">
                  <span className="w-28 shrink-0 font-property-label text-[11px] uppercase tracking-widest text-property-ink-variant">
                    {label}
                  </span>
            ...
            <div className="mt-auto pt-8">
              <div className="mb-6 flex items-end gap-8">
                <div>
                  <p className="mb-1 font-property-label text-[10px] uppercase tracking-widest text-property-ink-variant">
                    Member
                  </p>
      ...
      <div className="border-t border-property-ink/10">
        <div className="mx-auto max-w-property-max px-property-gutter py-10 text-center">
          <Link
            href="/packsaddle/events"
            className="font-property-label text-[13px] uppercase tracking-widest text-property-ink transition-colors hover:text-property-accent"
          >

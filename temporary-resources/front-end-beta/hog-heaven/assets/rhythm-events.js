...
(function(){
  ...
  const LABEL = 'text-white';   // on the accent fill, only white clears 4.5
  const btn = pad => `inline-block text-center bg-accent ${LABEL} ${pad} font-eyebrow font-semibold text-[13px] uppercase tracking-[0.1em] hover:bg-accent-dark transition-colors duration-300`;
  ...
  function renderProgNotice(){ const el = document.getElementById('progNotice'); if(!el) return;
    ...
    el.innerHTML = `<div class="flex items-center justify-center gap-4 flex-wrap">
      ...
      <button id="clearProg" class="inline-flex items-center gap-1 font-label-mono text-[11px] uppercase tracking-widest text-ink hover:text-on-surface-variant transition-colors"><span class="material-symbols-outlined text-[16px]">close</span> Clear</button>
    </div>`;
  ...
  function fbtn(v, active){ const on = v===active;
    return `<button data-v="${esc(v)}" class="font-label-mono text-label-mono px-6 py-2 border uppercase transition-all ${on ? 'border-accent bg-accent text-white' : 'border-ink/25 text-ink/80 hover:border-ink hover:text-ink'}">${esc(v)}</button>`; }
  ...
  function renderList(){
      ...
      if(f){ fw.style.display='';
        fw.innerHTML = `<div class="group"><div class="flex flex-col lg:flex-row border border-ink/10 overflow-hidden bg-white">
          <div class="lg:w-7/12 relative overflow-hidden h-96 lg:h-auto min-h-[360px]">${img(f,'absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700')}
            <div class="absolute top-8 left-8 bg-accent ${LABEL} px-4 py-1 font-label-mono text-label-mono uppercase tracking-widest">Featured Event</div></div>
          <div class="lg:w-5/12 p-12 flex flex-col justify-start">
            <span class="font-label-mono text-label-mono text-accent-deep mb-4 uppercase">${esc(fmtDate(f.date))}</span>
    ...
    listEl.innerHTML = list.map(e => `<div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-center border-b border-ink/10 pb-12 group">
      ...
      <div class="md:col-span-5"><span class="font-label-mono text-[11px] text-accent-deep uppercase tracking-[0.2em] mb-2 block">${catLine(e)}</span>
        <a href="event-detail.html?id=${encodeURIComponent(e.id)}" class="block"><h4 class="font-headline-md text-[28px] uppercase group-hover:text-accent transition-colors ${HEAD}">${esc(e.name)}</h4></a>
  ...
  async function renderDetail(){
    ...
    if(!e){ if(crumb) crumb.textContent='Event not found';
      root.innerHTML = `<p class="font-body-md text-on-surface-variant py-10">Sorry, we couldn't find that event. <a href="events.html" class="text-ink underline decoration-accent decoration-2 underline-offset-4">Back to the calendar</a>.</p>`; return; }
    ...
    const exp = (e.what_to_expect||[]).map(x=>`<li class="flex gap-3"><span class="text-accent">&bull;</span>${esc(x)}</li>`).join('');
    const gear = ((e.type==='Training' || e.type==='Standing') && (e.required_gear||[]).length) ? (e.required_gear).map(x=>`<li class="flex gap-3"><span class="text-accent">&bull;</span>${esc(x)}</li>`).join('') : '';
  ...
  async function renderHomeFeatured(){
    ...
    el.innerHTML = feat.map(e => `<a href="event-detail.html?id=${encodeURIComponent(e.id)}" class="group flex flex-col border border-ink/10 bg-surface-bright transition-colors duration-300 hover:border-accent/60 overflow-hidden">
      ...
      <div class="p-8 flex flex-col flex-grow">
        ...
        <h3 class="font-headline-md text-3xl ${HEAD} uppercase mb-4 group-hover:text-accent transition-colors">${esc(e.name)}</h3>

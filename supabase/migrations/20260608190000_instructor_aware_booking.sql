-- Make create_public_booking instructor-aware — Phase C of the
-- instructor-scheduling plan. Reproduces 20260607140000's function verbatim
-- except for three changes:
--   1. Use the property's timezone instead of the hard-coded 'America/Chicago'.
--   2. Auto-assign (private_lesson, no instructor picked) now selects the first
--      active instructor who is QUALIFIED for the chosen disciplines AND free
--      for this exact slot (schedule + travel buffer + capacity), not merely
--      "first active".
--   3. When a specific instructor IS picked, validate qualification + slot
--      availability before inserting, so a stale/forged client can't book an
--      unqualified or off-schedule instructor. The no_instructor_overlap
--      constraint + the travel-buffer trigger remain the final race guards.

...
import type { PublicImageStorage } from "@/lib/storage/public-image-storage";

// Validate + store a single public image (adventure imagery, instructor
// photo, …), returning its public URL. Single responsibility: it does not
// know about HTTP, FormData, auth, or WHICH bucket — the Server Action
// handles those and injects a storage adapter already pinned to a bucket
...
export async function uploadPublicImage(
  ...
  { bytes, contentType }: UploadPublicImageInput,
  ...
  // Random, unguessable path; foldered by uuid prefix so the bucket stays
  // browsable. No original filename (avoids collisions and path-traversal
  // surprises).
  const id = randomUUID();
  ...
  try {
    ...
    console.error("[upload-public-image] storage upload failed", err);
